...
There is an important distinction between an *exemption from an immunization* and an *immunzation that was not administered*. An exemption in this instance means that an individual's circumstances do not align with public health guidance and therefore they have a recognized "out" from public health policy requirements. It is not merely that a immunization was not administered, but the option of doing so was evaluated and decided to be impracticable for that individual. Therefore, they are *exempted* from the immunziation.

The type and reason that an individual *did not received an immunization* may be noted within a patient's electronic health record's immunization records. Both the type, which could range from a relatively benign value like "Contraindicated" or the more charged term, "Refused," and the reason, like "Patient Currently Pregnant" can be quite personal. This implementation guide is solely intended to note an exemption from an immunization, and if necessary the broad reason (generic exemption, medical, or vaccine-not-available), rather than particular type of decision and the specific reason for not receiving an immunization.
