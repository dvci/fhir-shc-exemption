...
The FHIR payload within an Exemption SMART Health Card SHALL be [small enough](https://spec.smarthealth.cards/#health-cards-are-small) to allow the entirety of the Exemption SMART Health Card to fit within [a single Version 22 QR code](https://spec.smarthealth.cards/#chunking). This limits the amount of data that SHOULD be included in FHIR resources that appear in SMART Health Card payloads.
...
Bundles meant to populate the `fhirBundle` element of an Exemption SMART Health Card with a type of `https://smarthealth.cards#covid19` SHALL conform to one of the Bundles profiled in this IG.
